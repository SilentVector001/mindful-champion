...
export default function ProgressPage({ user }: ProgressPageProps) {
  ...
  const fetchProgressData = async () => {
    try {
      const [goalsRes, skillsRes, achievementsRes] = await Promise.all([
        fetch('/api/goals'),
        fetch('/api/skills'),
  ...
  // Calculate key metrics safely after mount
  ...
  const recentWinRate = mounted && recentMatches.length > 0 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-champion-green/5 dark:from-champion-charcoal dark:via-gray-900 dark:to-champion-green/5">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        {goals.length > 0 && (
          ...
          >
            <Card className="border-0 bg-white shadow-sm">
              ...
              <CardContent>
                <div className="space-y-4">
                  {goals.filter(g => g.status === 'ACTIVE').slice(0, 3).map((goal: any) => (
                    <div
                      key={goal.id}
                      className="p-4 bg-slate-50 rounded-lg hover:shadow-md transition-all"
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h4 className="font-semibold text-slate-900">{goal.title}</h4>
                          <p className="text-sm text-slate-600 mt-1">
                            {goal.milestones?.filter((m: any) => m.status === 'COMPLETED').length || 0} / {goal.milestones?.length || 0} milestones completed
                          </p>
                        </div>
                        <Badge className="bg-gradient-to-r from-teal-500 to-orange-500 text-white border-0">
        ...
        {goals.length === 0 && !loadingData && (
          <motion.div
            ...
            className="mt-8"
          >
            <Card className="border-0 bg-gradient-to-br from-teal-50 to-orange-50 shadow-sm">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-slate-700 mb-6 max-w-md mx-auto">
                  Set your first goal and watch your progress come to life! Break it into milestones, track your journey, and celebrate every win.
                </p>
                <Link href="/progress/goals">
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-slate-900 mb-2">🎯 Your Journey Summary</h3>
                  <p className="text-slate-700 mb-3">
                    {mounted && goals.length > 0 ? (
                      <>
                        You're making great progress! {goals.filter(g => g.status === 'ACTIVE').length} active goals in motion, 
                        with an average completion of {Math.round(goals.reduce((sum, g) => sum + g.progress, 0) / goals.length)}%. 
                        {recentWinRate > winRate && ` Your win rate has improved by ${Math.abs(recentWinRate - winRate).toFixed(1)}% recently!`}
                        {skillData[3].value < 50 && " Focus on your third shot drop to reach the next level."}
                      </>
                    ) : (
                      <>
      ...
      {/* Persistent Avatar */}
      <AvatarCoach userName={user?.firstName || user?.name?.split(' ')[0] || 'Champion'} context="progress" />
    </div>
  )
}

...
export default function AIVideoPlayer({
  ...
  // Fallback to mark video as ready after URL is loaded
  useEffect(() => {
    if (!actualVideoUrl || !videoRef.current) return

    const video = videoRef.current
    
    // Set a fallback timeout to mark video as ready if events don't fire
    const fallbackTimer = setTimeout(() => {
      if (video.readyState >= 2 && !videoError) {
        // readyState >= 2 means HAVE_CURRENT_DATA or better
        console.log('[VideoPlayer] Fallback: Marking video as ready (readyState:', video.readyState, ')')
  ...
  useEffect(() => {
    ...
    const handleCanPlayThrough = () => {
      // Video can play through without buffering - definitely ready
      setIsVideoLoaded(true)
      setIsVideoReady(true)
      setVideoError(null)
    }
    ...
    const handleLoadedData = () => {
      // Video data is loaded - mark as ready
      setIsVideoLoaded(true)
      setIsVideoReady(true)
      setVideoError(null)
    }

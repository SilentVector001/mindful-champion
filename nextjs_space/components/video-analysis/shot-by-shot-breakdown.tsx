...
import {
  ...
  MapPin, Brain, Lightbulb, Gauge, Users, Crosshair, Route, Footprints, Loader2
} from 'lucide-react'
...
interface ShotMoment {
  id: number
  timestamp: number // Changed from 'time' to 'timestamp' for consistency
  type: string
...
export default function ShotByBreakdown({ videoUrl, analysisData, className }: ShotByBreakdownProps) {
  ...
  // Extract detected shots from analysis data with proper safety checks
  const detectedShots = (analysisData?.detectedShots && Array.isArray(analysisData.detectedShots)) 
    ? analysisData.detectedShots 
    : []
  ...
  const allShotMoments: ShotMoment[] = Array.isArray(detectedShots) 
    ? detectedShots.map((shot: any) => ({
        id: shot.id || 0,
        timestamp: shot.timestamp || shot.time || 0, // Support both 'timestamp' and 'time' fields
        type: shot.type || 'Unknown Shot',
        quality: shot.quality || 'good',
        score: shot.score || 70,
        thumbnail: '/placeholder-shot.jpg',
        analysis: {
          technique: shot.analysis?.technique || shot.analysis || 'Shot executed',
  ...
  // Sort and limit to top 5 key shots based on quality and score
  const shotMoments: ShotMoment[] = [...allShotMoments]
    .sort((a, b) => {
      // First sort by quality (excellent > good > needs_work)
      const qualityA = qualityPriority[a.quality as keyof typeof qualityPriority] || 0
      const qualityB = qualityPriority[b.quality as keyof typeof qualityPriority] || 0
      const priorityDiff = qualityB - qualityA
      if (priorityDiff !== 0) return priorityDiff
      
      // If same quality, sort by score (higher is better)
      const scoreDiff = b.score - a.score
      if (scoreDiff !== 0) return scoreDiff
      
      // If same quality and score, sort by timestamp (earlier first)
      return a.timestamp - b.timestamp
    })
    .slice(0, 5) // Limit to top 5 key shots
      ...
      <Card className="bg-slate-800/50 border-slate-700 shadow-2xl shadow-emerald-500/10">
        <CardHeader>
          ...
          <CardDescription className="flex items-center gap-2">
            ...
            Top 5 key shots identified • Click any shot for detailed analysis
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="relative aspect-video bg-slate-900">
                    ...
                    {thumbnails[shot.id] ? (
                      <Image
                        ...
                        alt={`${shot.type} at ${shot.timestamp}s`}
                        fill
                    ...
                    <div className="absolute top-2 left-2 bg-black/90 backdrop-blur-sm px-2 py-1 rounded-md text-xs text-white font-mono border border-white/10">
                      {Math.floor(shot.timestamp / 60)}:{(shot.timestamp % 60).toFixed(1).padStart(4, '0')}
                    </div>
                    {/* Quality badge */}
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-700">
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={shotMoments.findIndex(s => s.id === selectedShot.id) === 0}
                    onClick={() => {
                      const currentIndex = shotMoments.findIndex(s => s.id === selectedShot.id)
                      if (currentIndex > 0) {
                        jumpToShot(shotMoments[currentIndex - 1])
                      }
                    }}
                  >
                  ...
                  <span className="text-sm text-slate-400">
                    Key Shot {shotMoments.findIndex(s => s.id === selectedShot.id) + 1} of {shotMoments.length}
                  </span>
                  
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={shotMoments.findIndex(s => s.id === selectedShot.id) === shotMoments.length - 1}
                    onClick={() => {
                      const currentIndex = shotMoments.findIndex(s => s.id === selectedShot.id)
                      if (currentIndex < shotMoments.length - 1) {
                        jumpToShot(shotMoments[currentIndex + 1])
                      }
                    }}
                  >

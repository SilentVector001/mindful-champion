...
export default function AIInsightsPanel({ 
  ...
  return (
    <div className={cn("space-y-4", className)}>
      {/* Header - Hidden since parent already has header */}
                    ...
                    {insight.actionable && (
                      <div className="relative group/btn">
                        <Button 
                          size="sm" 
                          variant="outline"
                          className="h-7 text-xs bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 hover:text-emerald-300 hover:border-emerald-400/50 transition-all"
                          onClick={(e) => {
                            e.stopPropagation()
                        ...
                        {/* Tooltip */}
                        <div className="absolute bottom-full right-0 mb-2 px-3 py-2 bg-slate-900 text-white text-xs rounded-lg opacity-0 group-hover/btn:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10 shadow-lg border border-slate-700">
                          Click to go to {insight.action?.label || 'this feature'}

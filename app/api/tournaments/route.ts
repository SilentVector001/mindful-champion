...
export async function GET(request: NextRequest) {
  try {
    ...
    if (stateParam) {
      ...
      // If it's a full name, convert to abbreviation (case-insensitive)
      const stateAbbr = stateParam.length === 2 
        ? stateParam.toUpperCase() 
        : (STATE_NAME_TO_ABBR[stateParam] || 
           STATE_NAME_TO_ABBR[stateParam.charAt(0).toUpperCase() + stateParam.slice(1).toLowerCase()] || 
           stateParam.toUpperCase())
